...
2. The `feedback` object from the response contains a pre-signed URL for each key. For example, the `feedback` object is:
    ...
    ```
3. Request the returned URL (e.g. `POST /api/v1/feedback/tokens/{token_id}`) to associate the feedback key with the trace generated from the Agent Server run. For more details, refer to the [LangSmith API reference](https://api.smith.langchain.com/redoc).
...
When the user chooses a feedback option, `POST` to the corresponding URL. `GET` is also supported. See the [LangSmith API reference](https://api.smith.langchain.com/redoc) for more details.
