...
<Info>
The [organization user role](/langsmith/rbac#organization-user) is only available in organizations on plans with multiple workspaces. In organizations limited to a single workspace, all users are [organization admins](/langsmith/rbac#organization-admin). Custom organization-scoped roles are not available.
</Info>
...
For users that are already members of an organization, a workspace admin may add them to a workspace in the **Workspace members** tab on the [**Workspaces settings** page](https://smith.langchain.com/settings/workspaces). Users may also be invited directly to one or more workspaces when they are [invited to an organization](#manage-users).
