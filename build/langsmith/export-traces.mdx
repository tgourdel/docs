...
The time window is determined by `end_time - start_time`. If `end_time` is not provided, the current time is used. Queries without a `start_time` are treated as large time window queries.

To stay within these limits:
- Use time windows of 7 days or less when possible.
- Avoid full-text search (`filter='search("...")'`) when other filter types work.

When you exceed these limits, the API returns a `429 Too Many Requests` response. For general rate limit information, see [Rate limits](/langsmith/administration-overview#rate-limits).
