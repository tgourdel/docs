...
* <Icon icon="stack-push" size={16} /> [**Use multiple streaming modes**](#stream-multiple-modes)—choose from `values` (full state), `updates` (state deltas), `messages` (LLM tokens + metadata), `custom` (arbitrary user data), `checkpoints`, `tasks`, or `debug` (detailed traces).
* <Icon icon="arrows-sort" size={16} /> [**Use v2 streaming format**](#v2-streaming-format)—opt into a unified, type-safe streaming format with `version="v2"` (requires LangGraph >= 1.1).
...
| `checkpoints` | Streams checkpoint events in the same format as `get_state()`. Requires a checkpointer. |
| `tasks` | Streams task start and finish events, including results and errors. Requires a checkpointer. |
| `debug` | Streams as much information as possible throughout the execution of the graph (combines `checkpoints` and `tasks` events with additional metadata). |
...
<Note>
Requires LangGraph >= 1.1. All examples in this page use `version="v2"`.
</Note>
...
### Subgraph streaming with v2

With v2, all subgraph events use the same `StreamPart` format. The `ns` field identifies the source graph:
...
The following table summarizes the format changes when migrating from v1 to v2:
