...
3. **Need cross-invocation persistence?** Use `checkpointer: true`. LangGraph assigns each invocation a position-based namespace suffix (`calling_node`, `calling_node|1`, etc.) to prevent conflicts. For stable, name-based namespaces, wrap each subgraph with a unique node name — see [parallel subgraphs](/oss/javascript/langgraph/use-subgraphs#parallel-subgraphs).
