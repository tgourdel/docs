...
On first launch, the CLI automatically selects a model based on which API keys are available. After that, it remembers the most recently used model. You can also [set a persistent default](/oss/deepagents/cli/providers#setting-a-default-model) or pass `--model` at launch:
...
For full details on switching models, setting a default, configuring custom providers, and using the `~/.deepagents/config.toml` config file, see [Custom model providers](/oss/deepagents/cli/providers).
